...
class Network(nn.Module):
    ...
    def forward(self, image, im_info, gt_boxes=None):
        ...
        for i in range(5):
            assert torch.isfinite(fpn_fms[i]).all(), f'fpn feature map {i} corrupted'
...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_reg_list, anchors_list, gt_boxes, im_info):
        ...
        assert torch.isfinite(all_pred_cls).all(), 'all_pred_cls is corrupted'
        assert torch.isfinite(all_pred_dist).all(), 'all_pred_dist is corrupted'
