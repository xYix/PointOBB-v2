...
# model settings — same backbone/FPN as PseudoLabelHead, but VPD-enhanced
model = dict(
    type='RotatedFCOS',
    backbone=dict(
        type='ResNet',
    ...
    train_cfg=dict(
        ...
        multiple_factor=1/4,
        sigma_power=1.0,  # thresh3 *= (sigma/sigma_center)^power; 0=disabled
        mu_refine_radius=0,  # 0=no mu, 1=mu boundary check only, >1=mu center refinement+boundary
        ),
    test_cfg=dict(
        store_dir=store_dir,
        nms_pre=2000,
